...
import type { PipelineStep, StepStartData, StepCompleteData, StepErrorData, PipelineCompleteData, ValidationResult } from '../types/pipeline';
...
export function usePipeline(): UsePipelineReturn {
  ...
  const startPipeline = useCallback((url: string) => {
    ...
    connectPipelineSSE(url, {
      ...
      onPipelineComplete: (data: PipelineCompleteData) => {
        if (data.conference) {
          setConference(data.conference as unknown as Conference);
        }
        if (data.validation) {
  ...
  return { steps, conference, validation, isRunning, error, startPipeline, cancelPipeline };
}
